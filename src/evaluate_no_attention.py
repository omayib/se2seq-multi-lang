...
if __name__ == "__main__":
    ...
    model_path_file = os.path.join(model_save_dir, NORMAL_CHECKPOINT)
    ...
    decoder_multi_lang_eval = DecoderRNN(hidden_size=HIDDEN_SIZE,
    ...
    decoder_multi_lang_eval.load_state_dict(dencoder_cp)

    encoder_multi_lang_eval.eval()
    decoder_multi_lang_eval.eval()

    # Create a Predictor instance for the multi-language model
    multi_lang_predictor = Predictor(encoder=encoder_multi_lang_eval, decoder=decoder_multi_lang_eval,
    ...
    with torch.no_grad():
        ...
        _, _, attention_weights_id= decoder_multi_lang_eval(encoder_outputs_id,encoder_hidden_id)
